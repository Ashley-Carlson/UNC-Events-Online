...
if(!empty($_GET['verify']) && isset($_GET['verify']))
{
    ...
    $stmt = $db->prepare("SELECT EXISTS(SELECT 1 FROM user WHERE verify = :verify_string) as exist");
    $stmt -> execute(array(':verify_string' => $verify_string));
    $exists = $stmt -> fetch(PDO::FETCH_ASSOC);

    if ($exists['exist'] == 1)
    {
        $stmt = $db->prepare("UPDATE user SET is_inactive = 0 WHERE verify = :verify_string");
        $stmt -> execute(array(':verify_string' => $verify_string));
